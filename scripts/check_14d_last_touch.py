...
attributed_touches AS (
    ...
    JOIN all_touches t 
     ...
     AND t.touch_at >= p.paid_at - INTERVAL '14 days'
),
final_attribution AS (
    SELECT 
        p.payment_id,
        p.amount,
        COALESCE(a.channel, 'Pure Organic (No touch in 14d)') as final_channel
    FROM success_payments p
...
df_14d = con.execute(query_14d).df()
print("Mutually Exclusive Last-Touch Attribution (14-Day Lookback):")
print(df_14d.to_markdown(index=False))
